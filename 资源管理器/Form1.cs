...
namespace FileExplorer
{
    public partial class Form1 : Form
    {
        ...
        {
            if (new Class2().CheckDll(Application.StartupPath + "\\ClassLibrary2.dll"))
            {
            ...
            else
            {
                try
                {
                    MessageBox.Show(string.Format(CultureInfo.CurrentCulture,"缺少一个库，库名为{0}，路径为{1}，请检查！", "ClassLibrary2.dll", Application.StartupPath));
                    MessageBox.Show("正在准备修复");
                    Class1 c = new Class1();
                    System.Threading.Thread t = new System.Threading.Thread(new System.Threading.ParameterizedThreadStart(c.Add));
                    if (!t.IsAlive)
                    {
                        t.Start();
                    }
                ...
                {
                    MessageBox.Show(string.Format(CultureInfo.CurrentCulture,"尝试修复时发生错误,错误信息为{0}", ex.Message));
                    Close();
                }
            }
            base.OnLoad(e);
        ...
        private void ListUpdate(string newPath)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        if (item.Text.EndsWith(".exe", StringComparison.OrdinalIgnoreCase))  //是程序
                        {
                    ...
                    foreach (FileInfo file in files)
                    {
                        ...
                        fileItem.SubItems.Add(file.Length.ToString(CultureInfo.CurrentCulture) + "字节");
        ...
        private void CopyFile(string source)
        {
            ...
            catch (System.Threading.ThreadStartException ex)
            {
                MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        private void CopyAll(DirectoryInfo source, DirectoryInfo target)
        {
            System.Threading.Thread t = new System.Threading.Thread(new System.Threading.ThreadStart(()=> {
                ...
                for (DirectoryInfo temp = target.Parent; temp != null; temp = temp.Parent)
                {
                    ...
                    {
                        MessageBox.Show("无法复制！目标文件夹是源文件夹的子目录！", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error,MessageBoxDefaultButton.Button1,MessageBoxOptions.DefaultDesktopOnly);
                        return;
                    }
                }
        ...
        private void CreateFolder()
        {
            try
            {
                ...
                System.Threading.Thread t = new System.Threading.Thread(new System.Threading.ThreadStart(()=> {
                    ...
                    listView1.Items.Add(newPath, "重命名" + (i - 1 == 0 ? "" : (i - 1).ToString(CultureInfo.CurrentCulture)), 2);
                }));
                if (!t.IsAlive)
                {
                    t.Start();
        ...
        {
            System.Threading.Thread t = new System.Threading.Thread(new System.Threading.ThreadStart(()=> { Process.Start("http://blog.csdn.net/zhengzhiren"); }));
            if (!t.IsAlive)
            {
                t.Start();
            }
